...
O OFF é um fluxo de modelo direto. Ele não recebe a raiz inteira `MarySettings.root` como diretório pesquisável, mas apenas os diretórios canônicos existentes:
...
Diretórios internos, temporários ou de índices (`.state`, `.env`, `TrabalhoVR` de outras conversas, `.trash`, logs, assets, bancos SQLite `.sqlite`, `ERP/releases` e `tools/vr-search.ps1`) são explicitamente proibidos e excluídos do acesso no OFF.
