...
# ===========================================================================
# Scenario 11 — Runner retry policy: ACP -32000 aborts, transient retries
# ===========================================================================

class TestRunnerAuthRetry:
    """ACP -32000 is authentication-required, never a transient error.

    The fan-out researcher must stop after exactly 1 attempt on -32000
    (semantic ``exc.code == -32000`` and legacy textual ``"ACP -32000"``),
    while an ordinary transient ProviderError keeps the existing retry
    policy (RESEARCH_ATTEMPTS attempts).
    """

    def _runner(self, tmp_path, ephemeral):
        from unittest.mock import MagicMock

        from vrsoft_extractor.mary.config import MarySettings
        from vrsoft_extractor.mary.execution import ExecutionRunner
        ...
        retrieval = MagicMock()
        retrieval.prompt_for_role.return_value = "contexto de evidencias"

        def _unexpected_buffered(*args, **kwargs):
            raise AssertionError("synthesis must not run when all researchers fail")
        ...
        return ExecutionRunner(
            ...
            buffered_turn_runner=_unexpected_buffered,
            looks_like_final_envelope=lambda text: False,
        )

    def _execute_single_module(self, runner, tmp_path, run_id):
        from vrsoft_extractor.mary.execution import ExecutionContext
        ...
        bundle = EvidenceBundle(
            ...
            candidates=(
                EvidenceCandidate(
                    evidence_id="wiki:base",
                    source="wiki",
                    source_id="wiki:base",
                    document_id=1,
                    chunk_id=0,
                    title="Titulo wiki:base",
                    heading="",
                    content_type="wiki",
                    module="Fiscal",
                    product="",
                    excerpt="Conteudo de wiki:base",
                    score=0.8,
                    local_path="",
                ),
            ),
        ...
        context = ExecutionContext(
            conversation_id="conv-auth-retry",
            run_id=run_id,
            workspace=tmp_path,
        )
    ...
    def test_acp_error_code_stops_after_one_attempt(self, tmp_path):
        """Semantic path: AcpError(method="authenticate", code=-32000) → 1 attempt."""
        from vrsoft_extractor.mary.antigravity_acp import AcpError

        calls = []

        def ephemeral(cid, run_id, agent_id, *args, **kwargs):
            calls.append(agent_id)
            raise AcpError("authenticate", -32000)

        runner = self._runner(tmp_path, ephemeral)
        self._execute_single_module(runner, tmp_path, "run-auth-code")
        assert len(calls) == 1

    def test_legacy_textual_marker_stops_after_one_attempt(self, tmp_path):
        """Fallback path: exception text containing "ACP -32000" → 1 attempt."""
        calls = []

        def ephemeral(cid, run_id, agent_id, *args, **kwargs):
            calls.append(agent_id)
            raise RuntimeError("Antigravity: falha em authenticate (ACP -32000).")

        runner = self._runner(tmp_path, ephemeral)
        self._execute_single_module(runner, tmp_path, "run-auth-text")
        assert len(calls) == 1
    ...
    def test_transient_error_keeps_retry_policy(self, tmp_path):
        ...
        def ephemeral(cid, run_id, agent_id, *args, **kwargs):
            calls.append(agent_id)
            raise ProviderError("timeout transitorio simulado")

        runner = self._runner(tmp_path, ephemeral)
        self._execute_single_module(runner, tmp_path, "run-transient")
        assert len(calls) == RESEARCH_ATTEMPTS
