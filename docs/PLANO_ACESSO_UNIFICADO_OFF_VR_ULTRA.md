...
| VR | As mesmas ferramentas e fontes | Recuperar evidências relevantes das quatro fontes antes da resposta | Uma resposta principal, com complementação por ferramentas quando necessária |
...
- VR: obter o bundle comum antes de `_enrich_prompt`, criação do contrato de resposta e registro das evidências pendentes. A trilha Java usa pergunta, identificadores, aplicativo selecionado e relevância, sem depender de ULTRA ou do toggle do agente profundo. Uma pergunta sustentada apenas por código precisa funcionar mesmo com Wiki/KB/Schema vazios ou com falha documental.
