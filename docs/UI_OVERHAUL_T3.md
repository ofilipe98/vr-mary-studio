...
| `.chat-markdown :not(pre)>code`: fundo `--muted`, borda `1px solid var(--contrast-border)`, raio `.375rem` e padding `.1rem .35rem` | O tema mapeia `inlineCodeSurface` para `muted` e `VrInlineChipLayer` repinta o chip arredondado atrás do texto (`Theme.inlineChip*`); a altura vertical segue a caixa da fonte de código + `2*(.1rem + 1px)` com o topo alinhado aos glifos (pílula uniforme de ~19.19px por linha visual, como o `box-decoration-break: clone` do navegador) e o padding horizontal é aproximado porque o layout de texto do Qt não reserva esse espaço. |
