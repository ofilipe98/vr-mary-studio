...
Estado atual (VRTOOL-001, 22/09/2026): o modo `vr` é tool-driven. O modelo principal recebe um contrato VR estável (identidade, fontes disponíveis e política de grounding) e decide consultar Wiki, KB, Schema e Código por `vr_sources`, `vr_search` e `vr_read`; não há retrieval automático antes da resposta nem bundle inicial preenchido por busca. `vr_search` sem `source` executa as quatro lanes determinísticas em paralelo (Wiki, KB, Schema e Código, em ordem fixa wiki, kb, schema, code) e devolve um único resultado consolidado; com `source`, consulta somente a fonte escolhida. O chat VR não expõe filtro por módulo: a unidade de consulta é a fonte. O modo `ultra` mantém o fan-out por fonte com agentes (Wiki, KB, Schema e o agente DEV Java opcional) e a síntese única validada. As evidências que alimentam validação e citações do VR são somente as efetivamente retornadas por `vr_search`/`vr_read` durante o turno. Prefetch/cache de consulta é otimização posterior, fora desta entrega.

Histórico (VRNORMAL-001-v2, anterior a VRTOOL-001): o comando público `pesquisa` foi removido do produto. O modo `vr` respondia em um único turno do modelo principal com retrieval direto pelas quatro fontes fixas (Wiki source-wide, KB, Schema e Código determinístico), sem agentes e sem separação por módulo. O modo `ultra` mantém o fan-out por fonte com agentes (Wiki, KB, Schema e o agente DEV Java opcional) e a síntese única validada. Esse contrato de retrieval antecipado foi substituído pelo contrato tool-driven descrito acima.
...
- Modos persistidos `off`, `vr` e `ultra`: `off` não executa retrieval automático nem fan-out — as fontes locais permanecem disponíveis sob demanda; `vr` responde em uma única chamada principal, com contrato VR estável e retrieval sob demanda pelas ferramentas `vr_sources`/`vr_search`/`vr_read`, sem agentes; `ultra` faz fan-out por fonte com agentes. Não existe comando público de pesquisa multiagente. (Histórico V2, anterior a VRTOOL-001: o modo `vr` pré-carregava as fontes fixas antes da chamada ao modelo.)
