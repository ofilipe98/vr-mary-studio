...
Clique em **Usar no Ultra** para adicionar a seleção ao contexto. Outros aplicativos permanecem na lista; adicionar o mesmo aplicativo substitui sua seleção anterior. Depois de importar um pacote VR com sucesso, a aplicação pergunta se o pacote completo deve virar o escopo principal do VR Ultra ou se a seleção continuará por aplicativo: **Usar pacote completo** substitui o escopo atual pelos aplicativos da composição importada; **Escolher por aplicativo** preserva o fluxo manual (aplicativo → versão → variante/origem → **Usar no Ultra**). Um pacote já importado também pode virar o escopo principal a qualquer momento pelo botão **Usar no Ultra** no cartão de **Pacotes de origem importados**. Em **VR Ultra**, confira a lista. É possível selecionar vários aplicativos, com uma versão/variante/origem por aplicativo.

A **Análise de código JAR** liga automaticamente quando todos os contextos escolhidos estiverem prontos (`ready`). A importação não inicia descompilação nem indexação; a ativação automática aguarda o catálogo e o índice. Desativar a análise manualmente cancela qualquer ativação automática pendente. Importar um JAR avulso, código descompilado ou pacote descompilado não abre a pergunta de escopo do Ultra.
